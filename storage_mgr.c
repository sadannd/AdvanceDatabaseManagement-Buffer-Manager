...
RC closePageFile(SM_FileHandle *storageManagerFileHandler)
{
    if (storageManagerFileHandler->mgmtInfo != NULL)
    {
        fclose(storageManagerFileHandler->mgmtInfo);
        my_File_Ptr = NULL;
        logger("Success", "Closed page file successfully");
        return RC_OK;
    }
    else
    {
